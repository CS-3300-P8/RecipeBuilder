...
To unload some of the burden from the frontend, we decided to hinge on extensive react.js routing to delegate operations to the backend. This allowed operations such as database and service calls to reside on the backend and away from the client browser. The resulting impact was that database and service functionality performed consistently as a single, reliable host machine performed these operations, instead of each client’s choice of browser. In addition, the approach proved to be more secure, as sensitive requests to the database and the OpenAI service would not be directly accessible by the user.

### Modulated Codebase 
Furthermore, to unload some of the burden from the developer, we decided to modulate the specific functionalities in our implementation of Project 2. To start, we made use of the mediator architecture to streamline communication between the frontend and backend. As such, the client contains a PantryMediator class that provides an abstraction of the backend for any client-side pantry modification requests. This has the effect of simplifying development for the front-end pages as repeated routing calls relating to modifying pantries now have a unified interface. Just as well, any maintenance to the routing logic will only need to occur in the PantryMediator class – reducing overall development time and human errors. On the backend, route handling logic is likewise centralized to a single file (index.js). In extension, such routing handling logic is separated from lower-level accesses to the external MongoDB database and to the OpenAI API. For MongoDB, the command architecture was utilized to modulate the database-specific logic in the form of individual “executables”. This, in turn, simplifies the route handling logic which can now call a single execute() function for each of these commands to run the desired database action. This also decouples the logic between route handling and database management which improves maintainability of the code. On the other hand, with OpenAI API, the Factory Architecture was utilized to allow for extensions to added on top of a stable code base. Classes for specific API services are developed on top of a general service class, which allows for future development to focus on prompt creation and leverage existing service-based implementation. 
